...
def run_streamlit_app():
    if st.session_state.page == "main":
        ...
        # 維修保養按鈕
        with col_btn1:
            # 確保 Streamlit 知道這個按鈕是第一個 (用於 CSS 選擇器)
            st.markdown(f"""
                <style>
                    /* 由於 Streamlit 會在每個按鈕外包一層 div[data-testid="stButton"]，我們需要更精確地定位 */
                    /* 在 Streamlit Re-run 循環中，我們不能將 style 放在這裡，否則會重複 */
                </style>
            """, unsafe_allow_html=True)
        ...
        # 合約資訊按鈕
        with col_btn2:
            if st.button(BUTTON_STYLE["contract"]["text"], key="contract_btn"):
                st.session_state.page = "contract"
                st.rerun()
        
        # 覆蓋按鈕樣式 (放在主頁面邏輯之後，避免與 re-run 循環衝突)
        # 由於 Streamlit 會重新渲染，將樣式定義放在頂部 CSS 塊或這裡來應用
        st.markdown(f"""
            <style>
                /* 維修保養按鈕樣式 */
        ...
        if not 合約清單.empty and not 合約內容.empty:
            if "ContractNo" in 合約清單.columns and "ContractNo" in 合約內容.columns:
                # 確保 ContractNo 是字串並去除空白，以便進行集合運算
                合約清單["ContractNo"] = 合約清單.get("ContractNo", pd.Series([])).astype(str).str.strip()
                合約內容["ContractNo"] = 合約內容.get("ContractNo", pd.Series([])).astype(str).str.strip()
                
                # 找出合約清單和合約內容都有的合約編號
                合約交集 = sorted(set(合約清單["ContractNo"]) & set(合約內容["ContractNo"]))
                合約總筆數 = len(合約清單) # 顯示清單總筆數
